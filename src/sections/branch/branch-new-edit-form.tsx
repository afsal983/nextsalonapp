  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      if(responseData?.status > 401 ) {
        enqueueSnackbar(currentBranch ? `${t('general.update_failed')}:${responseData.message}` : `${t('general.create_failed')}:${responseData.message}`, { variant: 'error' });
      } else {
        // Keep 500ms delay
        await new Promise((resolve) => setTimeout(resolve, 500));
        reset(); 
        enqueueSnackbar(currentBranch ? t('general.update_success') : t('general.create_success'), { variant: 'success' });
      ...
      <Grid container spacing={3}>
        <Grid xs={12} md={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              <RHFTextField name="name" label={t('salonapp.branch.brname')} helperText={t('salonapp.branch.bn_helper')}/>
              <RHFTextField name="reg_name" label={t('salonapp.branch.regname')} helperText={t('salonapp.branch.rn_helper')}/>
              <RHFTextField name="address" label={t('salonapp.branch.address')} helperText={t('salonapp.branch.add_helper')}/>
              <RHFTextField name="telephone" label={t('salonapp.branch.telephone')} helperText={t('salonapp.branch.tp_helper')}/>
              <RHFTextField name="taxid" label={t('salonapp.branch.taxid')} helperText={t('salonapp.branch.ti_helper')}/>
              <RHFSelect native name="org_id" label={t('salonapp.branch.orgname')} InputLabelProps={{ shrink: true }}>
              ...
              <RHFSelect native name="loc_id" label={t('salonapp.branch.locname')} InputLabelProps={{ shrink: true }}>
                <option key={0}>{ t('general.dropdown_select') }</option>
                {location.map((item) => (
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              ...
              >
                {!currentBranch ? t('salonapp.branch.create_branch') : t('salonapp.branch.save_branch')}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
