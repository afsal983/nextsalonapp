...
export default function EmployeeEditView ({ id }: Props) {
  ...
  const { data: employeeData , isLoading: isemployeeDataLoading} = useSWR( `/api/salonapp/employee/${id}`, fetcher)
  ...
  if (isemployeeDataLoading || isbranchesLoading || isusersLoading|| isserviceLoading ) return <div>Loading</div>
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      ...
      <EmployeeNewEditForm currentEmployee={employeeData?.data[0]} branches={branches.data} users={users.data} services={service.data}/>
    </Container>
  )
}
