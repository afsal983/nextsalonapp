...
export default function PaymentNewEditForm({
  ...
  const handleSelectPayment = useCallback(
    (index: number, option: string) => {
      const selected_payment = paymenttypes?.find(
        (payment) => payment.name === option
      );
      // console.log(selected_payment)
      values.payments.map((items: Payment, count: number) => {
        if (items?.value > 0 && index !== count) {
          setValue(`payments[${count}].value`, 0);
        }
        return null; // Or return undefined
      });
      setValue(`payments[${index}].value`, values.totalAmount);
      setValue(`payments[${index}].payment_type`, selected_payment?.id);
    },
  ...
  const handleChangeamount = useCallback(
    (
      ...
      setValue(`payments[${index}].value`, event.target.value);
    },
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <Stack direction="row">
          ...
          <Box sx={{ width: 160, typography: "subtitle2" }}>
            {currentPayment &&
              paymenttypes?.map(
                (paymenttype) =>
                  paymenttype.default_paymenttype && paymenttype.name
              )}
      ...
      <Dialog
        fullWidth
        maxWidth="xs"
        open={paymentview.value}
        onClose={paymentview.onFalse}
      >
        <Stack
          direction="row"
          alignItems="center"
        ...
        >
          <Typography variant="h6">
            {" "}
            Select Payment for {values.totalAmount}
          </Typography>
        </Stack>
        <Box gap={1} p={1} display="grid" gridTemplateColumns="repeat(3, 1fr)">
          {paymenttypes
            ?.filter((item) => !item.name.toLowerCase().includes("split"))
            .map((item, index) => (
              <Stack spacing={1}>
                <Paper
                ...
                <Stack spacing={2}>
                  <RHFTextField
                    ...
                    onChange={(event) =>
                      handleChangeamount(index, item.name, event)
                    }
                    type="number"
                  />
                  <RHFTextField
