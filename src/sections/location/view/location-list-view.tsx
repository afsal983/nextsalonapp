...
export default  function LocationListView () {
  ...
  const { data: location,isLoading: isservicecategoryLoading, error: errorB } = useSWR('/api/salonapp/location', fetcher);
  ...
  const handleEditRow = useCallback(
    (id: string) => {
      router.push(paths.dashboard.location.edit(id))
    },
    [router]
  )
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: t('salonapp.location.locations'), href: paths.dashboard.location.root },
            { name: t('general.list') }
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.location.new}
              variant="contained"
            ...
            <TableSelectedAction
              ...
              onSelectAllRows={(checked) => {
                table.onSelectAllRows(
                  checked,
                  dataFiltered.map((row) => row.loc_id)
                )
              }
              }
              action={
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <LocationTableRow
                        key={row.loc_id}
                        row={row}
                        selected={table.selected.includes(row.loc_id)}
                        onSelectRow={() => { table.onSelectRow(row.loc_id) }}
                        onDeleteRow={() => { handleDeleteRow(row.loc_id) }}
                        onEditRow={() => { handleEditRow(row.loc_id) }}
                      />
                    ))}
