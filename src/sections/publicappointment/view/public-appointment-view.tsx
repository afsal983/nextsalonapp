...
export default function TimeSlotEditView() {
  ...
  const { data, isLoading, error } = useSWR(`/api/public?name=${parts[2]}`, fetcher);
  ...
  const {
    ...
    error: ErrorC,
  } = useSWR(`/api/salonapp/publicappointment/categorizedproductlistforpublic`, fetcher);

  if (error || ErrorE || ErrorC) return <div>Failed to load</div>;
  if (isLoading || iscatLoading || isemployeeLoading) return <div>Loading...</div>;
