...
export default function SettingsGeneral( { currentSettings }: Props) {
  ...
  const UpdateUserSchema = Yup.object().shape({
    ...
    emailid: Yup.string().required('Email is required').email('Email must be a valid email address'),
...
useEffect(() => {
  ...
  const logofilename = currentSettings.find(item => item.name === "logofile")?.value || ''
        ...
        <Grid xs={12} md={8}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <RHFSelect native name="emailserver" label={t('general.product_type')} InputLabelProps={{ shrink: true }}>
