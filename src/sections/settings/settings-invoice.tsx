...
export default function SettingsInvoice( { currentSettings }: Props) {
  ...
  useEffect(() => {
    ...
    const poslogoconetntBase64 = currentSettings.find(item => item.name === "poslogocontent")?.value || ''
    const poslogologofilename = currentSettings.find(item => item.name === "poslogofile")?.value || ''
