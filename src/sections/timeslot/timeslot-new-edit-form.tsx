...
export default function TimeSlotNewEditForm ({ currentTimeSlot }: Props) {
  ...
  const NewProductSchema = Yup.object().shape({
    id: Yup.string(),
    name: Yup.string().required(t('salonapp.service.timeslot.name_fvalid_error')),
    desc: Yup.string(),
    starttime: Yup.date(),
    endtime: Yup.date(),
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      starttime: currentTimeSlot?.starttime && timeStringToDate(currentTimeSlot?.starttime) || new Date() ,
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      if(responseData?.status > 401 ) {
        enqueueSnackbar(currentTimeSlot ? `${t('general.update_failed')}:${responseData.message}` : `${t('general.create_failed')}:${responseData.message}`, { variant: 'error' });
      } else {
        // Keep 500ms delay
        await new Promise((resolve) => setTimeout(resolve, 500));
        reset(); 
        enqueueSnackbar(currentTimeSlot ? t('general.update_success') : t('general.create_success'), { variant: 'success' });
      ...
      <Grid container spacing={3}>
        <Grid xs={12} md={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              <RHFTextField name="name" label={t('salonapp.timeslot.timeslot_name')} helperText={t('salonapp.timeslot.tn_helper')}/>
              <RHFTextField name="desc" label={t('salonapp.timeslot.desc')} helperText={t('salonapp.timeslot.desc_helper')}/>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              ...
              >
                {!currentTimeSlot ? t('salonapp.timeslot.create_timeslot') : t('salonapp.timeslot.save_timeslot')}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
