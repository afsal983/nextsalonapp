...
export default function UserEditView({ id }: Props) {
  ...
  const { data: userData, error: userError } = useSWR(
    `/api/salonapp/user/${id}`,
    fetcher
  );
  const { data: userroleData, error: categoryError } = useSWR(
    `/api/salonapp/userrole`,
    fetcher
  );
  const { data: branches, error: branchError } = useSWR(
    "/api/salonapp/branches",
    fetcher
  );

  if (userError || categoryError || branchError)
