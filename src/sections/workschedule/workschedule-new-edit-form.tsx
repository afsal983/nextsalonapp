...
export default function WorkScheduleNewEditForm({ currentWorkSchedule,timeSlot, employee }: Props ) {
...
const dayScheduleSchema = Yup.object().shape({
  ...
  slots: Yup.array().of(slotSchema).required('Slots are required').min(1, 'At least one slot is required'),
});
...
const employeeScheduleSchema = Yup.object().shape({
  ...
  dayschedule: Yup.array().of(dayScheduleSchema).required('Day schedule is required').min(1, 'At least one day schedule is required'),
});

const schedule = Yup.object().shape({
  employeeschedule: Yup.array().of(employeeScheduleSchema).required('Slots are required').min(1, 'At least one slot is required'),
});
...
const defaultValues = useMemo(
  ...
  const handleCreateAndSend = handleSubmit(async (data) => {
    ...
    try {
        ...
        if(responseData?.status > 401 ) {
          enqueueSnackbar(currentWorkSchedule ? responseData?.message : responseData?.message, { variant: 'error' });
        } else {
          // Keep 500ms delay
          await new Promise((resolve) => setTimeout(resolve, 500));
          reset(); 
          enqueueSnackbar(currentWorkSchedule ? t('general.update_success') : t('general.create_success'), { variant: 'success' });
  ...
  return (
    <FormProvider methods={methods}>
      <Card>
        <WorkScheduleNewEditDetails currentWorkSchedule={currentWorkSchedule} timeSlot={timeSlot} employee={employee}/>  
      </Card>
